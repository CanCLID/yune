...
| `scripts/typeduck-wasm-build.sh` with Emscripten env | Pass | Fresh `yune-typeduck.js` and `yune-typeduck.wasm` copied to `apps/yune-web/source/public/`. |
| `npm.cmd --prefix apps\yune-web\source run build` | Pass | Vite build and worker bundle passed. |
| `npm.cmd run test:e2e -- --grep "M28 FOLLOW-UP" --workers=1` from `apps\yune-web\e2e` | Pass | 2 browser tests passed in 28.3s. |
| `git apply --reverse --check ..\patches\yune-web-runtime.patch` from TypeDuck-Web source | Pass | Checked against the edited nested source checkout. |
| `git apply --check ...\apps\yune-web\patches\yune-web-runtime.patch` from a clean temporary source worktree | Pass | Worktree used locked TypeDuck-Web `03f9afd2cf6ca75653197f2193f24d1cd0adbd83` with schema submodule initialized. |
| `git diff --check` | Pass | Exit 0; PowerShell reported line-ending normalization warnings for edited docs only. |

## Evidence

- Hybrid oracle capture: `apps/yune-web/e2e/results/m28-follow-up-upstream-jyutping/oracle-capture.md`
- Target decision: `apps/yune-web/e2e/results/m28-follow-up-upstream-jyutping/target-decision.md`
- Ranking diagnosis: `apps/yune-web/e2e/results/m28-follow-up-upstream-jyutping/phrase-prefix-diagnosis.md`
- Browser Space/default-confirm evidence: `apps/yune-web/e2e/results/m28-follow-up-upstream-jyutping/browser-space-default-confirm.json`
- Browser ranking evidence: `apps/yune-web/e2e/results/m28-follow-up-upstream-jyutping/browser-upstream-ranking.json`
