 ...
 *
 * Prerequisites:
 * 1. Patched yune-web source (patches/yune-web-runtime.patch applied)
...
test.describe("yune-web Browser E2E", () => {
  ...
  test("M20 showcase control surface exposes honest controls", async ({ page }) => {
    ...
    await expect(page.locator("label").filter({ hasText: /ascii_punct/i })).toHaveCount(0);
    await expect(page.getByLabel(/ascii_punct/i)).toHaveCount(0);
    const commonCustom = await readRepoText("apps/yune-web/source/public/schema/common.custom.yaml");
    const commonYaml = await readRepoText("apps/yune-web/source/public/schema/common.yaml");
  ...
  test("M20 display controls change rendering and record mobile-only side-lookup limits", async ({ page }) => {
    ...
    const mobileSchema = await readRepoText("apps/yune-web/source/public/schema/jyut6ping3_mobile.schema.yaml");
