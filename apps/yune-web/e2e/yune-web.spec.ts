...
test.describe("yune-web Browser E2E", () => {
  ...
  test("M31 PUBLIC startup uses pruned public assets and warm cache @smoke @public-smoke", async ({ page }) => {
    ...
    expect(resources.some((resource) => /cangjie|loengfan|10keys|longpress/.test(resource.name))).toBe(false);

    await saveM31Json("startup-assets-cache.json", {
      startup,
      reloadStartup,
      resources,
      claimBoundary: "delivery cache and asset pruning only; not a browser startup speedup claim",
    });
  ...
  test("M31 PUBLIC output standards are browser-visible and AI stays default-off @smoke @public-smoke", async ({ page }) => {
    ...
    const unexpectedRemoteCalls = resources.filter((name) => /openai|anthropic|telemetry|analytics|segment|sentry/i.test(name));
  ...
  test("M25 DOGFOOD-11 visible lookup candidates expose dictionary details", async ({ page }) => {
    ...
    for (const { text, definition } of expectedLookupRows) {
      ...
      await expect(locator.locator(".candidate-definition")).toContainText(definition);
      await locator.hover();
  ...
  test("M24 dogfood UI uses only local Tailwind components", async ({ page }) => {
    const packageJson = JSON.parse(await readRepoText("apps/yune-web/package.json")) as {
    ...
    const tailwindConfig = await readRepoText("apps/yune-web/tailwind.config.ts");
  ...
  test("M20 User Dictionary persists schema customization", async ({ page }) => {
    ...
    await saveJsonEvidence("m20-input-memory-persistence-state.json", {
      ...
      browserSurface: {
        ...
        reason: "The browser persists translator/enable_user_dict=false and translator/encode_commit_history=false, but the current no-crates yune-web surface cannot suppress an already learned prefix prediction from candidate output.",
        engineCoverage: "Userdb learning and per-entry pronunciation behavior remain engine-proven in cantonese_parity and frontend_client userdb tests; this M20 follow-up does not change crates/ or add a yune_web_* export.",
  ...
  test("M16 schema menu and userdb pronunciation limits are explicit", async ({ page }) => {
    ...
    await saveJsonEvidence("m16-documented-gaps-state.json", {
      ...
      schemaMenu: {
        ...
        browserSurface: "M22 yune-web exposes a schema switcher for the browser playground schemas; M14 RimeGetSchemaList remains the ABI oracle evidence.",
        visibleSchemaControls,
      },
      userdbPronunciation: {
        oracleSurface: userdbFixture["capture"],
        browserSurface: "No yune-web native inspection surface exposes fork-only per-entry pronunciation levers.",
      },
    });
  ...
  test("Deploy returns visible success/error evidence", async ({ page }) => {
    ...
    const deployNotification = await page.locator(".yd-toast, .toast, [data-deploy-status], .notification").first().textContent({ timeout: 1000 }).catch(() => null);
  ...
  test("Customize returns visible success/error evidence", async ({ page }) => {
    ...
    if (await settingsPanel.count() > 0) {
      ...
      const customizeNotification = await page.locator(".yd-toast, .toast, [data-customize-status], .notification").first().textContent({ timeout: 1000 }).catch(() => null);
