...
pub fn rime_dict_rebuild_plan(
    ...
    if !input.source_available {
        ...
        let poet_reusable = !input.poet_required
            || (input.prebuilt_poet_available
                && input.poet_dict_file_checksum == Some(table_checksum));
    ...
    let poet_stale =
        input.poet_required && input.poet_dict_file_checksum != Some(dict_file_checksum);
    ...
    if input.table_dict_file_checksum.is_none()
        || (input.poet_required && input.poet_dict_file_checksum.is_none())
    {
        rebuild_table = true;
    }
