...
pub unsafe extern "C" fn RimeConfigSetInt(
    ...
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_set(config, key, Value::String(value.to_string()))
    })
}
...
pub unsafe extern "C" fn RimeConfigSetDouble(
    ...
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_set(config, key, Value::String(format!("{value:.6}")))
    })
}
...
pub unsafe extern "C" fn RimeConfigListAppendInt(
    ...
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_list_append(config, key, Value::String(value.to_string()))
    })
}
...
pub unsafe extern "C" fn RimeConfigListAppendDouble(
    ...
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_list_append(config, key, Value::String(format!("{value:.6}")))
    })
}
...
pub unsafe extern "C" fn RimeConfigCreateList(config: *mut RimeConfig, key: *const c_char) -> Bool {
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_set(config, key, Value::Sequence(Vec::new()))
    })
}
...
pub unsafe extern "C" fn RimeConfigCreateMap(config: *mut RimeConfig, key: *const c_char) -> Bool {
    crate::ffi_guard::guard(FALSE, || unsafe {
        config_set(config, key, Value::Mapping(Mapping::new()))
    })
}
