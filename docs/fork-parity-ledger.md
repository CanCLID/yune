...
| Profile-specific tuning | Forks can tune ranking, correction, sentence, and prediction constants in shared C++ code for their own product. | Yune keeps upstream/default behavior separate from TypeDuck-profile behavior, even when both use the same Rust translator types. | TypeDuck-calibrated constants must be installed through an explicit profile predicate or typed translator config. A `TYPEDUCK_*` constant read unconditionally by shared core is a red flag unless upstream oracle evidence proves the behavior is global and the name is made neutral. **Open item:** the `21.0` sentence penalty is currently shared/unconditional, so default `luna_pinyin` inherits it — M23 WI-1 gates it (`plans/m23-plan-architecture-hardening.md`). |
| Core/input split | librime gears/processors live inside the C++ engine. | Yune's deterministic translators, filters, dictionaries, ranking, userdb model, and AI layer live mostly in `yune-core`; the RIME processor pipeline, schema install/config glue, sessions, and FFI table currently live in `yune-rime-api`. | Processor semantics in `yune-rime-api` are compatibility-centric current state, not the ideal long-term native-engine boundary. Extract processor semantics into `yune-core` when a non-librime frontend or iOS/native product path needs a Rust API that does not tunnel through the C ABI. The finishable hardening is **M23**; the processor extraction itself stays trigger-gated (D-28). |
| Schema model | librime loads schemas through its C++ config/deployer stack. | `crates/yune-schema` exists as a standalone parser crate, but current production schema parsing/install lives in `yune-rime-api` (`config.rs`, `schema_install.rs`, `schema_selection.rs`). | Do not treat `yune-schema` as the live source of truth until it is promoted into the production path; otherwise delete or clearly park it. **M23 WI-3 makes this call.** |
...
5. ⁵ M21-GAP-01 provided the future oracle-backed scenario: TypeDuck v1.1.2 composes `loengnincin`, `leoicijyu`, `loengjathau`, `geijatcin`, and `gamjatheoi` as top-1 dictionary sentences while pre-fix Yune composed high-frequency short-piece garbage. Yune now preserves a scoped log-space sentence word-penalty heuristic for the TypeDuck `jyut6ping3` profile path, effectively preferring fewer pieces with frequency as a tiebreak across the six locked fixture cases. This does not reopen broad fork ranking byte parity or M17 poet/octagram LM work; expand the composition corpus opportunistically when new product cases appear. **Accuracy note (2026-06-20 review):** the heuristic is implemented as a *shared, unconditional* constant (`TYPEDUCK_SENTENCE_WORD_PENALTY = 21.0`, `translator/mod.rs`), **not** actually behind a profile predicate, so default upstream schemas (e.g. `luna_pinyin`) inherit it whenever `enable_sentence` composes a candidate. M23 WI-1 makes the scoping real — profile-gate it like the M21-GAP-02 prediction limit, or prove it global and rename it neutrally — and must land before M19.
