...
**Status: planned / next.** M37 is the next engine milestone and must close hard gates from [`plans/active/m37-plan-engine-hyper-optimization.md`](./plans/active/m37-plan-engine-hyper-optimization.md): latency and memory attribution, byte-backed or interned product storage, native mmap/file-backed loading, page-bounded candidate materialization/context export, `hai` movement, product memory movement, behavior parity, honest claims, and final quality gates. These requirements are intentionally unchecked before implementation; M37 cannot close by attribution-only work, another `rsmarisa` no-go, an mmap no-go, or latency-only work that leaves product memory unattributed.

- [ ] **M37-ENGINE-01**: Phase 0 evidence splits Track B `hai` across process-key, translator lookup, completion enumeration, candidate materialization, global sort/top-K, filters, userdb merge, context snapshot, ABI allocation/free, and working-set owners, and produces a product memory-owner table covering table bytes, parsed storage, candidates/context, userdb, reverse lookup, ABI buffers, and allocator high-water. If `hai` or product memory is not explained, implementation continues.
- [ ] **M37-ENGINE-02**: The final product storage path is byte-backed, mapped, or interned enough to remove the M36-style owned no-marisa heap mirror and move memory. `rsmarisa` is the preferred implementation and must be tried against actual `jyut6ping3` and `jyut6ping3_scolar` table data; if a verified crate/platform blocker prevents safe use, a reviewed interned/byte-backed re-emit or owner-backed adapter can satisfy the gate only if it meets the same no-`SourceFallback`, no-heap-mirror, byte-parity, and memory-movement evidence.
- [ ] **M37-ENGINE-03**: Final product status proves fresh table/prism/reverse artifacts, no `SourceFallback`, and an active selected byte-backed or interned product path rather than silent fallback to the M36 owned no-marisa artifacts.
...
- [ ] **M37-ENGINE-08**: Track B product median working set and peak working set move materially from the M36 final `~741.5 MB` median row / `885.3 MB` peak baseline, and Track A working-set attribution is refreshed. If memory does not move, M37 stays open to the named memory owner unless a maintainer explicitly splits a follow-up milestone before closeout.
- [ ] **M37-ENGINE-09**: The final native product path reports mmap/file-backed loading for the selected hot storage bytes. If `rsmarisa` is selected, native evidence reports `mapping_mode=mmap` for the marisa string-table payload. If direct `Trie::mmap()` cannot safely own the required file slice or lifetime, M37 lands a reviewed local patch, fork, or owner-backed mmap adapter before closeout; owned-buffer/no-marisa/mmap-no-go results keep M37 open.
- [ ] **M37-ENGINE-10**: Public claims remain separated: native wins are not browser wins without rebuilt release WASM and real browser evidence, and Track A ratios remain comparison caveats unless they independently improve.
- [ ] **M37-ENGINE-11**: Final quality gates pass: `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, focused upstream and TypeDuck parity tests, `cargo test --workspace`, final native benchmarks, report/docs checks, `git diff --check`, and runtime/browser/patch gates when runtime-visible files change.
...
| M37-ENGINE-01 | M37 | Planned - Phase 0 attribution must explain Track B `hai` across key-path, lookup, materialization, sort/filter/userdb, context, ABI, and working-set owners, and must attribute product memory owners |
| M37-ENGINE-02 | M37 | Planned - selected product storage must be byte-backed, mapped, or interned, with `rsmarisa` preferred and fallback allowed only if it proves no heap mirror, byte parity, and memory movement |
| M37-ENGINE-03 | M37 | Planned - final product status must prove fresh table/prism/reverse artifacts, no `SourceFallback`, and active selected byte-backed or interned product path |
...
| M37-ENGINE-08 | M37 | Planned - Track B product memory must move from the M36 final baseline and Track A working-set attribution must be refreshed |
| M37-ENGINE-09 | M37 | Planned - native product rows must prove mmap/file-backed loading, with `mapping_mode=mmap` when `rsmarisa` is selected, or M37 stays open for a patch, fork, or owner-backed mmap adapter |
| M37-ENGINE-10 | M37 | Planned - public claims must separate native, browser, Track A, and Track B evidence |
| M37-ENGINE-11 | M37 | Planned - final fmt/clippy/tests/benchmarks/report/docs/diff gates must pass, with runtime/browser gates when runtime-visible files change |
