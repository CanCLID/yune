...
| Engine performance | M55 has resumed in Phase 3R for native Track A match-or-beat. Phase 1 attributed the old unclassified memory floor, Phase 2 proved a flag-on `YUNE-POET/1` memory win (`110.5 MB`) but failed the full M55 ratchet, and Phase 2R measured `YUNE-POET/2` with a compiled prefix index as still no-go. Phase 3R expanded sentence fixtures, measured graph/DP volume, and landed a first owned-path beam-rejection checkpoint: the default-owned 37-character Luna row is now `2.407x` vs the M52 `3.267x` ceiling, the 59-character row is `1.827x` vs `2.447x`, and Track A peak is `185,860,096 B` vs `198,000,000 B`. | Byte-backed poet consumption remains default-off. Active Phase 3R still needs further owned-path reduction because Tier M is `<=1.50x` for both long rows. Default-on still requires two consecutive full M55 ratchet passes and Track A memory `<=125 MB`, with no ceiling loosening or ABI widening. |
