...
1. **M47 iOS-budget native memory reduction (active)** - the now-concrete memory
   ...
   RED-04 made the reverse/UI translator an optional keyboard-profile pack, and
   RED-05 removed the deploy rebuild/high-water transient. The latest isolated
   `jyut6ping3_mobile` proxy is steady **56.9 MB** WS, **23.7 MB** private,
   **16.0 MB** allocator-live, and **78.4 MB** peak WS. The next reduction branch
   is the primary compact-table parse transient at `create_session`. Plan:
   [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md);
