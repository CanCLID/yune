...
and a follow-on compiled-log weight-domain error; five fixed-binary macOS rounds
now match both pinned-librime long pages exactly. The Mac-versus-Windows latency
comparison remains diagnostic only, with signed ceilings unchanged. The repair
does not close the six remaining rows in the seven-row Lane B ordering/admission lane
or redirect concurrent Windows M59 work. [Original diagnostic](./reports/evidence/m59-macos-librime-analysis-20260710/README.md);
