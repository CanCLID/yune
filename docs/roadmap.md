...
1. **M47 iOS-budget native memory reduction (active)** - the now-concrete memory
   ...
   RED-04 made the reverse/UI translator an optional keyboard-profile pack. The
   latest isolated `jyut6ping3_mobile` proxy is steady **58.5 MB** WS,
   **23.3 MB** private, **16.0 MB** allocator-live, and **81.0 MB** peak WS. The
   next reduction branch is peak transient investigation. Plan:
   [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md);
