...
| Engine performance | M55 has resumed in Phase 3R for native Track A match-or-beat. Phase 1 attributed the old unclassified memory floor, Phase 2 proved a flag-on `YUNE-POET/1` memory win (`110.5 MB`) but failed the full M55 ratchet, and Phase 2R measured `YUNE-POET/2` with a compiled prefix index as still no-go. Phase 3R expanded sentence fixtures, measured graph/DP volume, and has landed owned-path checkpoints for beam rejection, path-state materialization, DP state-vector propagation, and bounded-refresh incremental sentence scratch. The latest strict M55-green checkpoint adds a bounded byte-backed lookup-filter comment cache to keep Track B green: 37-character Luna `2.249x`, 59-character Luna `1.774x`, Track B product key sequence `309.382 us`, and Track A peak `186,105,856 B`. | Byte-backed poet consumption remains default-off. Active Phase 3R still needs further owned-path reduction because Tier M is `<=1.50x` for both long rows, and every landed step must keep Track B green. Default-on still requires two consecutive full M55 ratchet passes and Track A memory `<=125 MB`, with no ceiling loosening or ABI widening. |
