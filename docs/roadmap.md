...
1. **M47 iOS-budget native memory reduction (active)** - the now-concrete memory
   ...
   RED-02 moved parsed prism storage to byte-backed runtime reads, and RED-03
   made compact lookup-record retention optional for the keyboard-profile probe.
   The latest isolated `jyut6ping3_mobile` proxy is steady **69.4 MB** WS,
   **30.2 MB** private, **20.4 MB** allocator-live, and **80.8 MB** peak WS. The
   next reduction branch is reverse/UI lazy loading for
   `script_translator@luna_pinyin` / `luna_pinyin_yune_reverse`. Plan:
   [`plans/active/m47-plan-ios-budget-native-memory-reduction.md`](./plans/active/m47-plan-ios-budget-native-memory-reduction.md);
