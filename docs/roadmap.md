...
| Engine performance | M55 has resumed in Phase 3R for native Track A match-or-beat. Phase 1 attributed the old unclassified memory floor, Phase 2 proved a flag-on `YUNE-POET/1` memory win (`110.5 MB`) but failed the full M55 ratchet, and Phase 2R measured `YUNE-POET/2` with a compiled prefix index as still no-go. Phase 3R expanded sentence fixtures, measured graph/DP volume, and has landed owned-path checkpoints for beam rejection, path-state materialization, DP state-vector propagation, bounded-refresh incremental sentence scratch, owned lookup index range reuse, exact new-span table materialization, incremental vocabulary new-edge derivation, and borrowed incremental graph edges. The latest current-code strict M55-green checkpoint reports 37-character Luna `2.173x`, 59-character Luna `1.728x`, Track B product key sequence `310.318 us`, and Track A peak `185,991,168 B`; the previous vocabulary new-edge run remains the better same-run long-row ratio record at `2.119x` / `1.645x`. | Byte-backed poet consumption remains default-off. Active Phase 3R still needs further owned-path reduction because Tier M is `<=1.50x` for both long rows, and every landed step must keep Track B green. Default-on still requires two consecutive full M55 ratchet passes and Track A memory `<=125 MB`, with no ceiling loosening or ABI widening. |
...
| M55 | Reopened - Phase 3R active | Native Track A match-or-beat program reopened after the Phase 2 no-go closeout landed byte-backed poet default-on with M52 red. Step 0 makes `YUNE-POET/1` consumption default-off, preserves deploy emission/validation, restores the owned Luna fast path, and re-verifies M52 green on main. Phase 2R implements `YUNE-POET/2` with a compiled prefix index, but the flag-on full ratchet is still red, so byte-backed poet remains default-off. Phase 3R expanded fixtures, measured graph work volume, and is reducing the owned long-row path; the latest borrowed incremental graph-edge checkpoint is strict-M55-green (`2.173x` / `1.728x`) but still above Tier M for both long Luna rows. Byte-backed re-landing/default-on still requires two full M55 ratchet passes with byte-backing enabled and Track A memory `<=125 MB`. Plan: [`plans/active/m55-plan-native-track-a-match-or-beat-program.md`](./plans/active/m55-plan-native-track-a-match-or-beat-program.md). |
