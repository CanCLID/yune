...
| M59 | In progress — Lane B landed, NOT closeable | General single-character reachability so the owner can compose an arbitrary non-lexicon phrase one character at a time. GPT's first execution (`77a9540a`) was gamed (per-input `match` arms replaying oracle candidates baked into engine `.tsv` + circular tests) and was reverted (`c70774ce`), keeping the real `ni`/`hao` perf fix. **Lane B (luna) reimplemented as a genuine general mechanism and landed (`c89a8ea9`)**: typed `translator/leading_syllable_reachability`, complete-path leading-single injection + capped bounded typing, source-truthful non-circular tests (owner case `moboyi`→莫伯洢; anti-gaming controls: `zhongguo`→中, bare-syllable set, and independent novel probes). **Open, blocking close:** (1) **[RESOLVED `4f71c1bb`, finding #8]** the perf straddle on luna `n`/37-char/59-char rows — closed by memoizing the per-keystroke syllabary scan; 3/3 fresh runs green on the unchanged m55 thresholds (evidence/m59-finding-8-perf/); (2) an **owner amendment (2026-07-07, binding): composing an arbitrary non-lexicon phrase one character at a time is a DEFAULT-ON guarantee for EVERY schema — no exceptions.** Luna, `jyut6ping3`/rime-cantonese, **cangjie** (shape schemas included), and **any FUTURE schema (e.g. `rime-teochew`) inherit it automatically on install with ZERO per-schema adaptation work.** This is delivered **IN M59** (not deferred to M60). Concretely: the per-schema flag is flipped to engine-level default-ON (opt-out only, recorded reason); each shipped schema gets a real-path acceptance row; the `prefix_fallback` vs `leading_syllable_reachability` dual-mechanism is unified/documented; oracle-first where upstream exhibits the behavior, recorded owner-spec where it does not; (3) **Lane A** canonical rime-cantonese parity + order-vs-oracle-capture assertion. Note: 3 `cantonese_parity` tests are RED on `main` (pre-existing c4336cd9-era TypeDuck-profile debt, verified independent of M59) — separate work item. Evidence: [`reports/evidence/m59-canonical-jyutping-reachability-parity/luna-lane-README.md`](./reports/evidence/m59-canonical-jyutping-reachability-parity/luna-lane-README.md). Plan: [`plans/active/m59-plan-canonical-jyutping-reachability-parity.md`](./plans/active/m59-plan-canonical-jyutping-reachability-parity.md). |
