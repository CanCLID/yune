...
| Web harness startup and memory | M41 is complete for the tracked `apps/yune-web/` browser harness. WEB-01 is active as a sidecar for browser-only WASM linear-memory reservation and Jyutping payload work. The fresh 2026-06-27 comparator baseline records Yune public-demo `luna_pinyin` at `128.0 MiB` peak WASM linear memory versus My RIME `16.0 MiB`, and Yune public-demo Jyutping at `893.1 MiB` versus My RIME `68.0 MiB`; Yune Jyutping also loads `33.5 MiB` unique encoded resources versus My RIME `24.9 MiB`. | WEB-01 claims stay limited to browser linear-memory, payload, and worker-retention owners. Any native support-code touch must preserve M44 native guards and must not be used as a browser speed claim. |
