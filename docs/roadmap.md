...
| WEB-02 | Complete with measured blocker | Public-demo Jyutping browser owner classification: stale `Rime::Prism/3.0` assets force source fallback and `owned_heap` storage, naming the compiled-asset contract as the next branch. The browser high-water remains `893.1 MiB`; no memory win is claimed. |
| WEB-03 | Partial native/asset closeout | Engine fix `3ffd4b21` unblocked clean rebuilds. The asset slice regenerated launch assets for `jyut6ping3_mobile`, `cangjie5`, and `luna_pinyin`, shipped Cangjie compiled assets, refreshed manifests/dist, and proved native public-demo storage is byte-backed with no fallback rows. Browser remeasure remains blocked here by missing Emscripten, so `893.1 MiB` is still the last browser high-water and not claimed fixed. |
