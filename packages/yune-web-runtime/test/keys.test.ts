...
import { keyEventToRimeKey, RIME_KEY, RIME_MASK, YuneWebKeyError } from "../src/keys.js";
...
describe("keyEventToRimeKey", () => {
  ...
  it("maps Rime punctuation aliases emitted by yune-web", () => {
  ...
  it("maps yune-web keypad digit aliases to keypad keycodes", () => {
  ...
  it("throws a deterministic error for unknown multi-character keys", () => {
    expect(() => keyEventToRimeKey({ key: "UnidentifiedKey" })).toThrow(YuneWebKeyError);
    expect(() => keyEventToRimeKey({ key: "UnidentifiedKey" })).toThrow(
      "Unsupported YuneWeb key: UnidentifiedKey",
    );
  });
});
