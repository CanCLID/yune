...
| `npm.cmd --prefix third_party/typeduck-web/source run build` | Passed: Vite build completed in 685ms; `public/worker.js` built at 33.5KB. |
| `$env:TYPEDUCK_APP_URL='http://localhost:5173/web/'; $env:M29_EVIDENCE_LABEL='after'; npm.cmd --prefix third_party\typeduck-web\e2e run test:e2e -- --grep "M29 PERF" --workers=1` | Passed: 1 Playwright test, 17.3s. |
...
| Check | Result |
| --- | --- |
| Regenerated `third_party/typeduck-web/patches/yune-typeduck-runtime.patch` from `third_party/typeduck-web/source` with `--binary --submodule=diff`. | Passed; only expected line-ending warnings. |
| `git -C third_party\typeduck-web\source apply --reverse --check ..\patches\yune-typeduck-runtime.patch` | Passed. |
| Clean detached worktree plus `git apply --check third_party/typeduck-web/patches/yune-typeduck-runtime.patch` after `schema` submodule init. | Passed. |
